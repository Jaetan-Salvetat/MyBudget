...
construction. La mesure indépendante de cet étage est bench/gemini.py
...
from reference.flow import AUTO_STAGES, CONFIRM, FlowPolicy, decide
from reference.structure import ExtractedReceipt, extract_from_result

OCR_DIRS = [RESULTS_DIR / "device_fr", RESULTS_DIR / "device_fr_big"]
ENHANCED_DIR = RESULTS_DIR / "device_fr_enhanced"
CLOUD_DIR = RESULTS_DIR / "llm_gemini37_flash"
GOLDEN_TEST_DIR = GOLDEN_DIR / "T1-test"
...
def covered_receipts() -> list[tuple[str, Path, Path]]:
    ...
    for ocr_dir in OCR_DIRS:
        for result in sorted(ocr_dir.glob("fr_genuine_*.json")):
            ...
            golden = GOLDEN_TEST_DIR / f"{doc_id}.json"
            if golden.exists():
