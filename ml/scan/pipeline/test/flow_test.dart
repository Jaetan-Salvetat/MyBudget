...
void main() {
  group('local stages', () {
    ...
    test('retry rescues failed first pass', () {
      final outcome = decide(receipt([2.0], 5.0), receipt([2.0, 3.0], 5.0), strict);
      expect(outcome.stage, FlowStage.localRetry);
    });

    test('confirm prefills with retry when tried', () {
      final outcome = decide(receipt([2.0], 9.0), receipt([2.0, 3.0], 10.0), strict);
  ...
  group('retry value guard', () {
    ...
    test('retry with equal value still validates', () {
      final outcome = decide(receipt([2.0, 3.0], 6.0), receipt([2.0, 3.0], 5.0), policy);
      expect(outcome.stage, FlowStage.localRetry);
    });
  ...
  group('rescue', () {
    test('rescue is not consulted when rules verify', () {
      var called = false;
      decide(receipt([2.0], 2.0), null, strict, rescue: () {
        called = true;
        return null;
      });
    ...
    test('rescue returning null falls through to confirm', () {
      final outcome = decide(receipt([2.0], 9.0), null, strict, rescue: () => null);
      expect(outcome.stage, FlowStage.confirm);
    });
  });
}
