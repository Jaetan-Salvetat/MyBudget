...
def _cross_check(split: str, doc_id: str, annotation: dict) -> bool | None:
    txt = ROOT / "dataset_findit" / split / "txt" / f"{doc_id}.txt"
    if not txt.exists():
        return None
...
def _annotate_one(entry: tuple[str, str], api_key: str) -> str | None:
    ...
    if annotation is None:
        image = ROOT / "dataset_findit" / split / "img" / f"{doc_id}.jpg"
        try:
        ...
        provenance = "api"
    record = {
        "source": f"dataset_findit/{split}/img/{doc_id}.jpg",
        "annotator": MODEL,
...
def main() -> None:
    ...
    for split in SPLITS:
        ...
        for image in sorted((ROOT / "dataset_findit" / split / "img").glob("*.jpg")):
