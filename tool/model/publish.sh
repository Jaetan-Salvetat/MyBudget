...
#
#   ./tool/model/publish.sh            # version suivante d'apres tool/model/lock.env
#   ./tool/model/publish.sh v5         # version imposee
#
# Sources par defaut : ml/classifier/output/model.onnx et
# ml/classifier/output/best/tokenizer.json, surchargeables par MODEL_SOURCE et
...
# modele dans le manifeste des assets, et tool/model/lock.env est reecrit ici.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"

source tool/model/lock.env

MODEL_SOURCE="${MODEL_SOURCE:-ml/classifier/output/model.onnx}"
TOKENIZER_SOURCE="${TOKENIZER_SOURCE:-ml/classifier/output/best/tokenizer.json}"
TOKENIZER_ASSET="assets/models/tokenizer.bin"

if [ $# -gt 1 ]; then
  echo "usage: ./tool/model/publish.sh [version]" >&2
  exit 64
fi
...
if [ -f "$MODEL_SOURCE" ]; then
  if [ -f "$TOKENIZER_SOURCE" ]; then
    ...
    dart run tool/model/build_tokenizer_asset.dart "$TOKENIZER_SOURCE" "$TOKENIZER_ASSET"
  else
...
Récupéré au build par \`./tool/model/fetch.sh\`, épinglé dans \`tool/model/lock.env\`."

cat > tool/model/lock.env <<EOF
...
# tool/model/fetch.sh et par le cache de la CI — versionne dans git, il rend
# chaque commit reproductible : un vieux commit recupere le modele qu'il attend.
#
# Genere par tool/model/publish.sh, a ne pas editer a la main.
...
echo "  tool/model/lock.env  -> $RELEASE"
