...
class _RevenuesListState extends ConsumerState<RevenuesList> {
  ...
  Widget build(BuildContext context) {
    ...
    final isLoading = ref.watch(revenueProvider).isLoading;
    ...
    if (!_filterData.isEmpty) {
      filteredRevenues = revenues.where((revenue) {
        if (_filterData.searchQuery != null && _filterData.searchQuery!.isNotEmpty &&
            !revenue.name.toLowerCase().contains(_filterData.searchQuery!.toLowerCase())) {
          return false;
        }
        if (_filterData.minAmount != null && revenue.amount < _filterData.minAmount!) return false;
        if (_filterData.maxAmount != null && revenue.amount > _filterData.maxAmount!) return false;
        if (_filterData.accountIds.isNotEmpty && !_filterData.accountIds.contains(revenue.accountId)) return false;
        if (_filterData.beneficiaryIds.isNotEmpty && !_filterData.beneficiaryIds.contains(revenue.beneficiaryId)) return false;
        return true;
      }).toList();
    }
    ...
    return ListView.builder(
      ...
      padding: const EdgeInsets.only(top: 120, bottom: 145, left: 16, right: 16),
      itemCount: items.length,
      itemBuilder: (context, index) {
        final item = items[index];
        if (item == 'HEADER') {
          return _buildHeaderContainer(context, filteredRevenues, isEmpty);
        }
        if (item == 'DIVIDER') {
          return Padding(
            ...
            child: Row(
              children: [
                ...
                Padding(
                  ...
                  child: Text('Revenus passés', style: TextStyle(
        ...
        return RevenueCard(
          ...
          onDelete: () => ref.read(revenueProvider.notifier).deleteRevenue(revenue.id),
          onEdit: () {
            RevenueBottomSheet.show(
              context: context,
              accounts: accounts,
              revenue: revenue,
              onSubmit: (updatedRevenue) { ref.read(revenueProvider.notifier).updateRevenue(updatedRevenue); },
              onCancel: () {},
            );
          },
        );
  ...
  Widget _buildHeaderContainer(BuildContext context, List<RevenueModel> displayedRevenues, bool isEmpty) {
    final monthlyRevenues = ref.read(revenueProvider.notifier).getMonthlyRevenues();
    final fixedRevenues = ref.read(revenueProvider.notifier).getMonthlyFixedRevenues();
    final punctualRevenues = ref.read(revenueProvider.notifier).getMonthlyPunctualRevenues();
    return Column(
      children: [
        Container(
    ...
    return Padding(
      ...
      child: AnimatedSwitcher(
        ...
        transitionBuilder: (Widget child, Animation<double> animation) {
          ...
          final beginOffset = isSearch ? const Offset(0.2, 0.0) : const Offset(-0.2, 0.0);
          final offsetAnimation = Tween<Offset>(begin: beginOffset, end: Offset.zero).animate(animation);
        ...
        child: _isSearchVisible
                  ...
                  children: [
                    Expanded(
                      child: FrostedTextField(
                        ...
                        onChanged: (value) { setState(() { _filterData.searchQuery = value; }); },
                        autofocus: true,
                      ),
                    ),
                    IconButton(
                      icon: const Icon(Icons.close),
                      onPressed: () { setState(() { _isSearchVisible = false; _searchController.clear(); _filterData.searchQuery = ''; }); },
                    ),
                  ],
                ),
              )
            : SizedBox(
                ...
                child: Row(
                  ...
                  children: [
                    Text('Mes revenus', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold)),
                    Row(
                      children: [
                        IconButton(
                        ...
                        IconButton(
                          icon: Icon(Icons.filter_list, color: _filterData.isEmpty ? Theme.of(context).iconTheme.color : Theme.of(context).colorScheme.primary),
    ...
    return EmptyState(
      ...
      onPressed: () {
        ...
        RevenueBottomSheet.show(
          context: context,
          accounts: accounts,
          onSubmit: (newRevenue) { ref.read(revenueProvider.notifier).addRevenue(newRevenue); },
          onCancel: () {},
        );
      },
    );
  ...
  void _showFilterBottomSheet(BuildContext context) {
    ...
    RevenueFilterBottomSheet.show(
      ...
      onApply: (updatedFilterData) { setState(() { _filterData = updatedFilterData; }); },
      onClear: () { setState(() { _filterData = RevenueFilterData(); }); },
      onCancel: () {},
    );
  }
}
