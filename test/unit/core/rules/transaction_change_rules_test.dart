...
void main() {
  ...
  group('the terms of a rule', () {
    test('are untouched when nothing but the category moved', () {
      expect(
        TransactionChangeRules.changesTerms(
          expense(),
          expense(categorySlug: 'logement.charges'),
        ),
        isFalse,
    ...
    test('move with the price', () {
      expect(
        TransactionChangeRules.changesTerms(expense(), expense(amount: 900)),
        isTrue,
      );
    });
    ...
    test('move with the account', () {
      expect(
        TransactionChangeRules.changesTerms(expense(), expense(accountId: 2)),
        isTrue,
      );
    });
